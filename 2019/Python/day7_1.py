...
def take_input(ptr, prgm, value):
    ...
    print(f"OpCode 3, taking input : {value}")
    position = prgm[ptr + 1]
    ...
    while prgm[ip] != 99:
        ...
        match op.code:
            ...
            case 3:
                ...
                print(phase_value)
                ip, prgm = take_input(ip, prgm, phase_value.pop(0))
            case 4:
                if DEBUG: print(f" [ OUTPUT ] at [ {ip} ]")
                ip, prgm, out = give_output(op, ip, prgm)
            case 5:
...
simulations = [ list(_) for _ in permutations(phase_values, 5) ]
...
for simulation in list(simulations):
    ...
    for _ in range(len(simulation)):
        result, phase_setting = run([ simulation.pop(0), phase_setting ], program.copy())

    # results.append([ phase_setting, sim ])
    if phase_setting > max_result:
        max_result = phase_setting

print("Maximum phase value =", max_result)
